...
/*   Updated: 2025/11/06 13:58:39 by viceda-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_slab(t_internal *slab, float *tnear, float *tfar)
{
...
static float	get_intersection_distance(float tnear, float tfar)
{
...
vector_dot(p, cube->param.axis[0]), vector_dot(ray.direction, \
cube->param.axis[0]), -cube->side / 2.0f, cube->side / 2.0f, 0, 0, 0};
	if (!check_slab(&slab, &tnear, &tfar))
		return (-1.0f);
	...
	if (!check_slab(&slab, &tnear, &tfar))
		return (-1.0f);
	return (get_intersection_distance(tnear, tfar));
}
